...
import com.mkaszycki.data.RoutingDataRepository
...
class DataModule {
    @Provides
    fun providesPoiRepository(poiService: PoiService): PoiRepository = PoiDataRepository(poiService)

    @Provides
    fun providesRoutingRepository(routingService: RoutingService): RoutingRepository =
        RoutingDataRepository(routingService)
}